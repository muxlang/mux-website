...
- **Durable Worker rate limiting.** Production chat requests use a per-IP
  Durable Object cooldown that is consistent across isolates and fail closed if
  the binding is unavailable. Local development retains its in-memory fallback.
...
extracted from the former monorepo. The site version lives in `package.json` and
is shown in the footer; it tracks the docs site, not the compiler.

### Added
- **Where-clause documentation**: The language guide now documents where-clause
  constraints (#19).
- **Monaco + Shiki keyword parity check**: CI parity-checks the Monaco and Shiki
  keyword sets against the canonical `syntax-matrix.json` so highlighting cannot
  drift from the spec (#10).
- **GitHub Pages deploy workflow**: Added a Pages deploy workflow (with job-scoped
  permissions).

### Changed
- **Search**: Replaced the search plugin with `@easyops-cn/docusaurus-search-local`
  (#23).
- **Cold-start UX**: Pre-warm the playground API and show a staged status message so
  the first compile feels responsive (#22).
- **Standalone repo setup**: Established as an independent repo with CLAUDE.md/AGENTS
  linking the muxlang/mux-context hub.

### Fixed
- **Terminal editor height cutoff** (#21).
- **Install command overflow** on the home screen (#20).
- **Ligatures and Ctrl handling** in the editor (#8).
