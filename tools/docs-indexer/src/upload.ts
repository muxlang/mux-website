...
export function writeNdjson(
  ...
  writeRecords(
    target.ndjsonPath,
    entries.map((entry) => toRecord(entry, target)),
  );
...
async function vectorizeRequest<T>(endpoint: string, init: RequestInit = {}): Promise<T> {
...
export async function deleteVectors(
  ...
  for (let i = 0; i < ids.length; i += VECTORIZE_DELETE_BATCH_SIZE) {
    ...
    report(
      `Deleting cleanup batch ${batchNumber}/${batchCount} (${batch.length} ${vectorLabel})...`,
    );
...
export async function publishIndex(
  ...
  await waitForReadiness(`upsert mutation ${mutationId}`, async () => {
